...
"%__SABIN%\dbisql" -q -nogui -c "uid=dba;pwd=appeon;dsn=SalesDB" %ASA_PATH%\MobiLink\setup\syncsa.sql
"%__SABIN%\dbisql" -q -nogui -c "uid=dba;pwd=appeon;dsn=SalesDB" read salesdb.sql 
"%__SABIN%\dbisql" -q -nogui -c "uid=dba;pwd=appeon;dsn=SalesDB" read mluser10.sql
goto dbend
:olddb
"%__SABIN%\dbisql" -q -nogui -c "uid=dba;pwd=appeon;dsn=SalesDB" read "%curr_dir%\salesdb.sql" 
"%__SABIN%\dbisql" -q -nogui -c "uid=dba;pwd=appeon;dsn=SalesDB" read "%curr_dir%\mluser9.sql" 
:dbend
if not errorlevel == 0 goto done
