$PBExportHeader$w_pbstyle_freeform.srw
forward
global type w_pbstyle_freeform from Window
end type
type dw_freeform from DataWindow within w_pbstyle_freeform
end type
end forward

global type w_pbstyle_freeform from Window
string MenuName = "m_pbstyle_freeform"
event getfirstrow ()
event getnextrow ()
event getpriorrow ()
event getlastrow ()
...
global w_pbstyle_freeform w_pbstyle_freeform
...
public function boolean Update_Row ()
...
If dw_freeform.AcceptText ( ) = 1 Then

	// Yes, turn off redraw for now
	dw_freeform.SetRedraw ( True )
 
	// Switch out of QueryMode
	dw_freeform.Modify ( "DataWindow.QueryMode = no" )
	...
	// Re-retrieve
	If  dw_freeform.Retrieve ( ) > 0 Then
		// Yes, clear any locks
		Commit; 
		// Turn redraw back on
		dw_freeform.SetRedraw ( True )
...
If ib_update = False And lb_ReSpecify = False Then
	dw_freeform.Modify ( "DataWindow.Readonly = yes")
...
// Prompt for save
If dw_freeform.ModifiedCount ( ) + dw_freeform.DeletedCount ( ) > 0 Then
...
If li_rc = 1 Then
	...
	If this.Update_Row ( ) = False Then
		Return
	End If
End If
 
dw_freeform.SetReDraw ( False )
// This will cause the datawindow to reset after using QueryMode  */
dw_freeform.DataObject = dw_freeform.DataObject
...
public function boolean Update_Row ();//*-----------------------------------------------------------------*/
//*    Update_Row routine:   Update Changes to Database
//*-----------------------------------------------------------------*/
string  ls_Describe

// See if we're in QueryMode
ls_Describe = dw_freeform.Describe ( "DataWindow.QueryMode" )

// Are we?
If ( ls_Describe = "no" ) Then

	// No, see if we're Updateable
	ls_Describe = dw_freeform.Describe ( "DataWindow.Table.UpdateTable" )

	// Are we?
	If ( ls_Describe <> "?" ) Then
		// Yes, did DataWindow update work?
		If ( dw_freeform.Update( ) = -1 ) Then
			// Rollback changes 
			Rollback;
			Return False
	   End If
...
If ib_update = False Then
	dw_freeform.Modify ( "DataWindow.Readonly = no" )
End If
...
dw_freeform.Modify ( "DataWindow.QueryMode = yes" )
...
If dw_freeform.SetSort ( ls_null ) <> -1 Then
	dw_freeform.Sort ( )
End If
end subroutine
...
// Get current row
ll_CurrentRow = dw_freeform.GetRow ( )
...
// Go to the next row 
dw_freeform.ScrollPriorRow ( )
end subroutine

public subroutine Print_Dialog ();//*-----------------------------------------------------------------*/
//*     Print_Dialog routine:   Invoke Print Dialog and Print
//*-----------------------------------------------------------------*/
If OpenWithParm ( w_pbstyle_dw_print_options, dw_freeform ) <> -1 Then
...
// Were we at the end?
If ( ll_CurrentRow = dw_freeform.RowCount(  ) ) Then
	// Yes, let the user know
...
dw_freeform.ScrollToRow ( 9999999 )

// Got any rows?
If dw_freeform.RowCount ( ) = 0 Then 
	// No, then issue error 
...
dw_freeform.ScrollToRow ( dw_freeform.InsertRow( 0 ) )

// Get the column with the lowest tab order
ls_Describe = dw_freeform.Describe ( "DataWindow.Column.Count" )
li_cols = Integer ( ls_Describe )

li_LowTab = -1
li_LowCol = 1
For li_i = 1 to li_cols
	ls_Describe = dw_freeform.Describe ( "#" + String ( li_i ) + ".TabSequence" )
...
dw_freeform.SetColumn ( li_LowCol )
dw_freeform.SetFocus ( )
end subroutine
...
ll_rows = dw_freeform.RowCount ( )

// Got any rows?
If ll_rows > 0 Then 
	// Yes, scroll to first one
	dw_freeform.ScrollToRow ( 1 ) 
Else
   // No, then issue error 
   this.Dynamic Event NoMoreRows ()
End If
...
If dw_freeform.SetFilter ( ls_null ) <> -1 Then
	dw_freeform.Filter ( )
End If
end subroutine
...
//*-----------------------------------------------------------------*/
dw_freeform.DeleteRow ( 0 )
end subroutine

public subroutine Clear_Filter ();//*-----------------------------------------------------------------*/
//*     Clear_Filter routine:   Reset the Filter
//*-----------------------------------------------------------------*/
dw_freeform.SetFilter ( "" )
dw_freeform.Filter ( )
end subroutine
...
If li_Result = 1 Then
	...
	li_Result = dw_freeform.ImportFile ( ls_PathName )
...
on w_pbstyle_freeform.create
if IsValid(this.MenuID) then destroy(this.MenuID)
if this.MenuName = "m_pbstyle_freeform" then this.MenuID = create m_pbstyle_freeform
this.dw_freeform = create dw_freeform
this.Control[]={this.dw_freeform}
end on

on w_pbstyle_freeform.destroy
if IsValid(this.MenuID) then destroy(this.MenuID)
...
If Not IsValid ( dw_freeform ) Then Return

// We have positive Y?
If dw_freeform.Y > 0 Then
	// Yes, then clear it
	li_Y = 0	
Else
...
dw_freeform.Move ( 0, li_Y )

// Resize the DataWindow to match the Form
dw_freeform.Resize ( this.WorkSpaceWidth ( ), WorkSpaceHeight ( ) )
end event
...
lm_menu.SetDWObject ( dw_freeform )

// Connect the DataWindow up to the transaction
dw_freeform.SetTransObject ( SQLCA )

// Set message title
dw_freeform.Modify ( "DataWindow.Message.Title=" + "'" + this.Title + "'" )
...
li_cols = Integer ( ls_Describe )
For li_i = 1 to li_cols
	ls_Describe = dw_freeform.Describe ("#" + String ( li_i ) + ".Update" )
...
// Force an accept 
li_update = dw_freeform.AcceptText ( ) 
...
If li_update <> -1 Then 
	...
	ll_TotalModified  = dw_freeform.DeletedCount ( )
	ll_TotalModified += dw_freeform.ModifiedCount ( )
