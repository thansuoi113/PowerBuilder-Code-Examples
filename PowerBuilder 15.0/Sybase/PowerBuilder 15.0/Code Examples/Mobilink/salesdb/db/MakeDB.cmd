...
"%__SABIN%\dbinit" -p 2048 "%curr_dir%\salesdb.db"
if not errorlevel == 0 goto done

echo ----- CREATE REMOTE DATABASE -----------------------------
"%__SABIN%\dbinit" -ea AES -ek secret -i -p 2048 "%curr_dir%\salesdb_remote.db"
if not errorlevel == 0 goto done

echo ----- SET UP DSN -----------------------------------------
"%__SABIN%\dbdsn" -du SalesDB -y
"%__SABIN%\dbdsn" -wu SalesDB -y -v -c "uid=dba;pwd=sql;eng=SalesDB;dbn=SalesDB;dbf=%curr_dir%\salesdb.db;links=shmem;start=%ASA_CMD% -c 8M -zl -ti 0" 
...
if "%ASA_CMD%" == "dbeng12" goto newdb
if "%ASA_CMD%" == "dbeng11" goto newdb
if "%ASA_CMD%" == "dbeng10" goto newdb
goto olddb
:newdb
...
"%__SABIN%\dbisql" -q -nogui -c "uid=dba;pwd=sql;dbkey=secret;dsn=SalesDB_remote" read salesdb_remote.sql
if not errorlevel == 0 goto done
...
:ASANY_not_set
echo environment variable SQLANY12/SQLANY11/SQLANY10/ASANY9/ASANY8 not set
