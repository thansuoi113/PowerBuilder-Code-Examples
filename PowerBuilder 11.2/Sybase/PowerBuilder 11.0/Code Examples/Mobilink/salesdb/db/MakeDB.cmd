...
if exist "%curr_dir%\salesdb.db" "%ASA_PATH%\win32\dberase" -y "%curr_dir%\salesdb.db"
if not errorlevel == 0 goto done

if exist "%curr_dir%\salesdb_remote.db" "%ASA_PATH%\win32\dberase" -y "%curr_dir%\salesdb_remote.db"
if not errorlevel == 0 goto done

echo ----- CREATE CONSOLIDATED DATABASE -----------------------
"%ASA_PATH%\win32\dbinit" -p 2048 "%curr_dir%\salesdb.db"
if not errorlevel == 0 goto done

echo ----- CREATE REMOTE DATABASE -----------------------------
"%ASA_PATH%\win32\dbinit" -ea AES -ek secret -i -p 2048 "%curr_dir%\salesdb_remote.db"
if not errorlevel == 0 goto done
...
"%ASA_PATH%\win32\dbisql" -q -nogui -c "uid=dba;pwd=sql;dsn=SalesDB" read "%curr_dir%\salesdb.sql" 
"%ASA_PATH%\win32\dbisql" -q -nogui -c "uid=dba;pwd=sql;dsn=SalesDB" read "%curr_dir%\mluser10.sql" 
goto dbend
:olddb
"%ASA_PATH%\win32\dbisql" -q -nogui -c "uid=dba;pwd=sql;dsn=SalesDB" read "%curr_dir%\salesdb.sql" 
"%ASA_PATH%\win32\dbisql" -q -nogui -c "uid=dba;pwd=sql;dsn=SalesDB" read "%curr_dir%\mluser9.sql" 
:dbend
if not errorlevel == 0 goto done

echo ----- SET UP REMOTE DATABASE -----------------------------
"%ASA_PATH%\win32\dbisql" -q -nogui -c "uid=dba;pwd=sql;dbkey=secret;dsn=SalesDB_remote" read "%curr_dir%\salesdb_remote.sql" 
if not errorlevel == 0 goto done
